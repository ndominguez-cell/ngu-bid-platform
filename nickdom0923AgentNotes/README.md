...
| [`21_WON_BIDS_CONTROL_LOOP.md`](./21_WON_BIDS_CONTROL_LOOP.md) | Claude + Codex | Append-only shared working doc reframing the platform as a tight "maximize *won* bids" control loop. Multi-tenant / "second customer" work is **paused on purpose** here — optimize the loop for NGU first. Win/loss capture (step 1) landed 2026-06-17. | Read for the current single-customer focus + the append log of loop changes. |
| [`22_SECURITY_RECONCILIATION_2026-06-18.md`](./22_SECURITY_RECONCILIATION_2026-06-18.md) | Claude | Reconciles a phone-drafted security doc against the **actual repo**: the draft's entire P0 (auth, RLS, profile role, seed) is **already done in code**; the real frontier is P1 (rate limiting, validation, headers, file/LLM hardening) + CodeRabbit, plus one deploy-state unknown (is RLS live in prod?). Corrects the draft; carries its tooling + open questions forward. | Read **before any new security sprint** — so you don't re-do finished P0 work. |
| [`24_SECURITY_AND_FEEDBACK_LOOP_INTEGRATION_2026-07-14.md`](./24_SECURITY_AND_FEEDBACK_LOOP_INTEGRATION_2026-07-14.md) | Claude | The record of folding the last two valuable branches into `main` (note-21 security fixes + token encryption; the win/loss feedback loop), how the estimator conflict was resolved, which migrations are already live, and the one — `security_followups.sql` (H1 grant-revoke) — that must run **only after** the code deploys. Also lists the branches now safe to delete. | Read **before touching branches, the estimator, or the H1 grant** — and before the next deploy. |
...
*Folder created 2026-05-27 by nickdom0923 via Claude (Cowork). Index table refreshed 2026-06-18 — now covers notes `00`–`22`, `teach-the-session.md`, and `PARTNER_MERGE_INSTRUCTIONS_2026-06-16.md`.*

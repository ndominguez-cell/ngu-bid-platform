...
| [`08_PRODUCTIZATION_PUNCHLIST.md`](./08_PRODUCTIZATION_PUNCHLIST.md) | Claude | The concrete pre-customer-#2 punch-list: lock down API routes, stop self-service role changes, enforce per-workspace RLS. | Read before the security sprint. |
| [`09_BRANCH_RESOLUTION_2026-06-10.md`](./09_BRANCH_RESOLUTION_2026-06-10.md) | Claude | Branch cleanup plan: which `claude/*` branches to keep vs delete, and why design-refresh should land via PR (not direct to `main`, which auto-deploys prod). | Read when tidying branches. |
| [`10_TENANCY_DECISION_AND_BOUTIQUE_VERTICAL_2026-06-13.md`](./10_TENANCY_DECISION_AND_BOUTIQUE_VERTICAL_2026-06-13.md) | Claude | Recommends keeping the pooled `workspace_id` + RLS tenancy model (vs project-per-company) for selling to many small businesses; introduces the **boutique** as the first non-construction vertical on the chassis; sketches the anonymized cross-tenant cost-prediction layer. Has open questions for the partner. | Read when discussing tenancy, generalization, or new verticals. |
...
*Folder created 2026-05-27 by nickdom0923 via Claude (Cowork).*
